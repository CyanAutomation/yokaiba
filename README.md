...
For GitHub deployment automation, configure a protected `production` environment containing `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`, plus `CLOUDFLARE_DEPLOYMENT_URL` as a repository or environment variable containing the canonical Worker origin (for example `https://yokaiba.example.workers.dev`). Restrict that environment to the `main` branch and require the appropriate reviewers. CI injects the package version and immutable Git SHA into the deployed Worker; `/healthz`, `/readyz`, and `/v1/version` expose them for support and cache diagnostics. The deploy workflow verifies health, readiness, version metadata, public puzzle redaction, conditional GET caching, and a deployed OpenAPI contract check after each deployment. Both deployment checks also require the deployed version and SHA to exactly match the commit released by the workflow.
