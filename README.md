...
The weekly Kaseki documentation sweep requires a stable HTTPS `KASEKI_BASE_URL` repository or organization variable and a `KASEKI_API_TOKEN` secret in a protected `kaseki-docs` environment. Restrict that environment to the default branch and its maintainers. The sweep uses the default branch, `README.md,docs/**/*.md`, and `npm run typecheck && npm test` when schedule-triggered; manual inputs override those values. GitHub Actions waits for Kaseki's terminal run status and fails when the agent fails or exceeds its three-hour budget.
